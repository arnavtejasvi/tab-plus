...
public class ProfileScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.text(font, Component.literal(playerName), cx - font.width(playerName) / 2, topY, 0xFFFFFF, true);
        ctx.text(font, Component.literal("— profile —"), cx - font.width("— profile —") / 2, topY + 10, 0x666666, true);

        int tagRowY = topY + ROW_HEIGHT * 2;
        ctx.text(font, Component.literal("Tag"), cx - font.width("Tag") / 2, tagRowY - 10, 0xAAAAAA, true);
        ...
        ctx.text(font, Component.literal(tagLabel), cx - font.width(tagLabel) / 2, tagRowY + 6, tagColor, true);

        int noteRowY = topY + ROW_HEIGHT * 4;
        ctx.text(font, Component.literal("Note"), cx - font.width("Note") / 2, noteRowY - 10, 0xAAAAAA, true);
    }
