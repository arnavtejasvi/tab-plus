...
public class TabOverlayRenderer {
    ...
    public static void render(GuiGraphicsExtractor ctx, int screenWidth,
        ...
        if (header != null) {
            ...
            ctx.fill(hx, startY - PADDING, hx + hw + PADDING * 2, startY + font.lineHeight + PADDING, BG_COLOR);
            ctx.text(font, header, hx + PADDING, startY, WHITE, true);
            startY += font.lineHeight + PADDING * 2 + 2;
        }

        ctx.fill(startX - PADDING, startY - PADDING,
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            if (i == selectedIndex) {
                ctx.fill(x - 1, y, x + entryWidth + 1, y + ROW_HEIGHT, HOVER_COLOR);
            }

            Identifier skinTex = entry.getSkin().body().texturePath();
            ctx.blit(RenderPipelines.GUI_TEXTURED, skinTex, x, y + 1, 8.0f, 8.0f, 8, 8, 64, 64);
            ctx.blit(RenderPipelines.GUI_TEXTURED_PREMULTIPLIED_ALPHA, skinTex, x, y + 1, 40.0f, 8.0f, 8, 8, 64, 64);

            int nameColor = data.tag != RelationshipTag.NONE ? data.tag.color : WHITE;
            Component displayText = getDisplayText(entry);
            ctx.text(font, displayText, x + HEAD_SIZE + HEAD_MARGIN, y + 1, nameColor, false);

            if (data.tag != RelationshipTag.NONE) {
                int dotX = x + HEAD_SIZE + HEAD_MARGIN - 3;
                ctx.fill(dotX, y + 3, dotX + 2, y + 5, data.tag.color | 0xFF000000);
            }

            int ping = entry.getLatency();
            String pingStr = ping < 0 ? "?" : ping + "ms";
            int pingColor = pingColor(ping);
            ctx.text(font, Component.literal(pingStr), x + entryWidth - font.width(pingStr), y + 1, pingColor, false);
        }
        ...
        if (!filterStr.isEmpty() || players.isEmpty()) {
            ...
            ctx.fill(fx, filterY - 2, fx + fw + PADDING * 2, filterY + font.lineHeight + 2, BG_COLOR);
            ctx.text(font, Component.literal(filterLabel), fx + PADDING, filterY, 0xFFDD44, false);
        }
        ...
        if (footer != null) {
            ...
            int fx = (screenWidth - fw - PADDING * 2) / 2;
            ctx.fill(fx, footerY - PADDING, fx + fw + PADDING * 2, footerY + font.lineHeight + PADDING, BG_COLOR);
            ctx.text(font, footer, fx + PADDING, footerY, WHITE, true);
        }
    }
